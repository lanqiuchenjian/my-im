...
public class CIMJavaClient999 implements CIMEventListener {
 	public static void startup() {
		 ...
		 * 第二步 设置全局的事件监听器
		 */
		CIMEventBroadcastReceiver.getInstance().setGlobalCIMEventListener(new CIMJavaClient999());
		...
		while (true) {
			...
			while (scanner.hasNext()) {
				...
				sentBody.put("fromAccount", "20000");
				sentBody.put("content", scanner.next());
	...
	public void onConnectFinished(boolean hasAutoBind) {
		...
		if(!hasAutoBind){
			CIMPushManager.bindAccount("20000");
		}
		/*
		 * 在此可以将事件分发到各个监听了CIMEventBroadcastReceiver的地方
		 * 第一步 连接到服务器 在需要监听事件的类调用CIMListenerManager.registerMessageListener(listener);
